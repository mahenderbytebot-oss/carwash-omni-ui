...
const EditVehicleModal: React.FC<EditVehicleModalProps> = ({ isOpen, vehicle, onClose, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    if (!formData.make || !formData.model || !formData.registrationNumber || !formData.parkingLocation) {
      setError('Make, Model, License Plate, and Parking Location are required.');
      return;
    }
      ...
      <IonContent className="ion-padding">
        <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          <IonList lines="full">
            ...
            <IonItem>
              <IonLabel position="stacked">Parking Location *</IonLabel>
              <IonInput 
                value={formData.parkingLocation} 
                onIonChange={e => handleChange('parkingLocation', e.detail.value!)} 
                placeholder="Ex. P2A10"
                required
              />
            </IonItem>
            <IonItem>
